...
class CFG:
    ...
    # ============== fold =============
    valid_id = '20231210121321-stk-w3-mean'
...
def read_image_mask(fragment_id,start_idx=27,end_idx=36):
    images = []

    idxs = range(start_idx, end_idx)
    for i in idxs:
        print(f"Loading image {(i+1) - start_idx} of {end_idx - start_idx}...")
        print(f"{CFG.comp_dataset_path}{CFG.scrolls_dir}/{fragment_id}/layers/{i:02}.tif")
        image = cv2.imread(f"{CFG.comp_dataset_path}{CFG.scrolls_dir}/{fragment_id}/layers/{i:02}.tif", 0)
    ...
    if fragment_id in ['20231022170901','20231022170900']:
        mask = cv2.imread(CFG.comp_dataset_path + f"{CFG.scrolls_dir}/{fragment_id}/{fragment_id}_inklabels.tiff", 0)
    else:
        mask = cv2.imread(CFG.comp_dataset_path + f"{CFG.scrolls_dir}/{fragment_id}/{fragment_id}_inklabels.png", 0)
    
    # mask = np.pad(mask, [(0, pad0), (0, pad1)], constant_values=0)
    fragment_mask=cv2.imread(CFG.comp_dataset_path + f"{CFG.scrolls_dir}/{fragment_id}/{fragment_id}_mask.png", 0)
                            ...
                            if not np.all(mask[a:a + CFG.tile_size, b:b + CFG.tile_size]<0.01):
                                if not np.any(fragment_mask[a:a+ CFG.tile_size, b:b + CFG.tile_size]==0):
                                    if (y1,y2,x1,x2) not in windows_dict:
                                        ...
                                        #print("cfg size, size, in_chans")
                                        #print(CFG.size,CFG.size,CFG.in_chans)
...
class CustomDataset(Dataset):
    ...
    def fourth_augment(self,image):
        ...
        cropping_num = random.randint(1, 8) # change to use start and end args
...
valid_mask_gt = cv2.imread(CFG.comp_dataset_path + f"{CFG.scrolls_dir}/{fragment_id}/{fragment_id}_inklabels.png", 0)
...
for fid in fragments:
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[ModelCheckpoint(filename=f'timesformer_wild15_{fid}_{fold}_fr_{enc}'+'{epoch}',dirpath=CFG.model_dir,monitor='train/total_loss',mode='min',save_top_k=12,every_n_epochs=5,save_on_train_epoch_end=True),CFG.epochs],
    )
